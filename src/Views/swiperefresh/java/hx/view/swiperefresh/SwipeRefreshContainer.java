...
public class SwipeRefreshContainer extends ViewGroup {

    private static final String TAG = SwipeRefreshContainer.class.getSimpleName();
    ...
    /**
     * <b>ATTRIBUTE:</b>
     * a switcher indicate whiter init more function is enabled
     */
    ...
    /**
     * <b>ATTRIBUTE:</b>
     * Scrolling duration status init more complete -> default
    ...
    /**
     * <b>ATTRIBUTE:</b>
     * Scrolling duration swiping to init more -> default
     */
    private int mSwipingToLoadMoreToDefaultScrollingDuration = DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION;

    /**
     * <b>ATTRIBUTE:</b>
     * Scrolling duration status default -> loading more, mainly for auto init more
    ...
    public SwipeRefreshContainer(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshContainer, defStyleAttr, 0);
        try {
            final int N = a.getIndexCount();
            for (int i = 0; i < N; i++) {
                int attr = a.getIndex(i);
                if (attr == R.styleable.SwipeRefreshContainer_refresh_enabled) {
                    setRefreshEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.SwipeRefreshContainer_load_more_enabled) {
                    setLoadMoreEnabled(a.getBoolean(attr, true));

                } else if (attr == R.styleable.SwipeRefreshContainer_swipe_style) {
                    setSwipeStyle(a.getInt(attr, STYLE.CLASSIC));

                } else if (attr == R.styleable.SwipeRefreshContainer_drag_ratio) {
                    setDragRatio(a.getFloat(attr, DEFAULT_DRAG_RATIO));

                } else if (attr == R.styleable.SwipeRefreshContainer_refresh_final_drag_offset) {
                    setRefreshFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshContainer_load_more_final_drag_offset) {
                    setLoadMoreFinalDragOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshContainer_refresh_trigger_offset) {
                    setRefreshTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshContainer_load_more_trigger_offset) {
                    setLoadMoreTriggerOffset(a.getDimensionPixelOffset(attr, 0));

                } else if (attr == R.styleable.SwipeRefreshContainer_swiping_to_refresh_to_default_scrolling_duration) {
                    setSwipingToRefreshToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_REFRESH_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_release_to_refreshing_scrolling_duration) {
                    setReleaseToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_refresh_complete_delay_duration) {
                    setRefreshCompleteDelayDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_refresh_complete_to_default_scrolling_duration) {
                    setRefreshCompleteToDefaultScrollingDuration(a.getInt(attr, DEFAULT_REFRESH_COMPLETE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_default_to_refreshing_scrolling_duration) {
                    setDefaultToRefreshingScrollingDuration(a.getInt(attr, DEFAULT_DEFAULT_TO_REFRESHING_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_swiping_to_load_more_to_default_scrolling_duration) {
                    setSwipingToLoadMoreToDefaultScrollingDuration(a.getInt(attr, DEFAULT_SWIPING_TO_LOAD_MORE_TO_DEFAULT_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_release_to_loading_more_scrolling_duration) {
                    setReleaseToLoadingMoreScrollingDuration(a.getInt(attr, DEFAULT_RELEASE_TO_LOADING_MORE_SCROLLING_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_load_more_complete_delay_duration) {
                    setLoadMoreCompleteDelayDuration(a.getInt(attr, DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION));

                } else if (attr == R.styleable.SwipeRefreshContainer_load_more_complete_to_default_scrolling_duration) {
    ...
    protected void onFinishInflate() {
        ...
        if (_sr_header != null && _sr_header instanceof SwipeTrigger) {
            _sr_header.setVisibility(GONE);
        }
        if (_sr_footer != null && _sr_footer instanceof SwipeTrigger) {
            _sr_footer.setVisibility(GONE);
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // header
        if (_sr_header != null) {
            final View headerView = _sr_header;
        ...
        // footer
        if (_sr_footer != null) {
            final View footerView = _sr_footer;
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        mHasHeaderView = (_sr_header != null);
        mHasFooterView = (_sr_footer != null);
    }
    ...
    protected ViewGroup.LayoutParams generateDefaultLayoutParams() {
        return new SwipeRefreshContainer.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
    }
    ...
    protected ViewGroup.LayoutParams generateLayoutParams(ViewGroup.LayoutParams p) {
        return new SwipeRefreshContainer.LayoutParams(p);
    }
    ...
    public ViewGroup.LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new SwipeRefreshContainer.LayoutParams(getContext(), attrs);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // refresh trigger condition        init more trigger condition

                if (triggerCondition) {
                    // if the refresh's or init more's trigger condition  is true,
    ...
    /**
     * is init more function is enabled
     *
     * @return
     */
    ...
    /**
     * switch init more function on or off
     *
     * @param enable
     */
    ...
    public void setRefreshHeaderView(View view) {
        if (view instanceof SwipeRefreshTrigger) {
            if (_sr_header != null && _sr_header != view) {
                removeView(_sr_header);
            }
            if (_sr_header != view) {
    ...
    /**
     * set init more footer view, the view must at least be an implement of SwipeLoadTrigger
     * the view can also implement {@code SwipeTrigger} for more extension functions
     *
     * @param view
     */
    public void setLoadMoreFooterView(View view) {
        if (view instanceof SwipeLoadMoreTrigger) {
            if (_sr_footer != null && _sr_footer != view) {
                removeView(_sr_footer);
            }
            if (_sr_footer != view) {
                this._sr_footer = view;
                addView(_sr_footer);
            }
        } else {
     ...
     * Default value is the init more footer view height {@link #mFooterHeight}<p/>
    ...
    /**
     * Set the final offset you can swipe to init more.<br/>
    ...
    /**
     * set an {@link OnLoadMoreListener} to listening init more event
     *
     * @param listener
     */
    ...
    public void setRefreshing(boolean refreshing) {
        if (!isRefreshEnabled() || _sr_header == null) {
            return;
        }
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onPrepare() {
            if (_sr_header != null && _sr_header instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                _sr_header.setVisibility(VISIBLE);
        ...
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (_sr_header != null && _sr_header instanceof SwipeTrigger && STATUS.isRefreshStatus(mStatus)) {
                if (_sr_header.getVisibility() != VISIBLE) {
                    _sr_header.setVisibility(VISIBLE);
                }
        ...
        public void onRefresh() {
            if (_sr_header != null && STATUS.isRefreshing(mStatus)) {
                if (_sr_header instanceof SwipeRefreshTrigger) {
                    ((SwipeRefreshTrigger) _sr_header).onRefresh();
                }
        ...
        public void onReset() {
            if (_sr_header != null && _sr_header instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                ((SwipeTrigger) _sr_header).onReset();
                _sr_header.setVisibility(GONE);
            }
        }
    };
    ...
    LoadMoreCallback mLoadMoreCallback = new LoadMoreCallback() {
        ...
        public void onPrepare() {
            if (_sr_footer != null && _sr_footer instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                _sr_footer.setVisibility(VISIBLE);
        ...
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (_sr_footer != null && _sr_footer instanceof SwipeTrigger && STATUS.isLoadMoreStatus(mStatus)) {
                if (_sr_footer.getVisibility() != VISIBLE) {
                    _sr_footer.setVisibility(VISIBLE);
                }
        ...
        public void onLoadMore() {
            if (_sr_footer != null && STATUS.isLoadingMore(mStatus)) {
                if (_sr_footer instanceof SwipeLoadMoreTrigger) {
                    ((SwipeLoadMoreTrigger) _sr_footer).onLoadMore();
                }
        ...
        public void onReset() {
            if (_sr_footer != null && _sr_footer instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                ((SwipeTrigger) _sr_footer).onReset();
                _sr_footer.setVisibility(GONE);
            }
        }
    };
    ...
    private class AutoScroller implements Runnable {
        ...
        public void run() {
            ...
            if (finish) {
                ...
                mmLastY = currY;
                SwipeRefreshContainer.this.autoScroll(yDiff);
                post(this);
            }
        }
